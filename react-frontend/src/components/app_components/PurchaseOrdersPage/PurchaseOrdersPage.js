...
import PurchaseOrdersDatatable from "./PurchaseOrdersDataTable";
import PurchaseOrdersEditDialogComponent from "./PurchaseOrdersEditDialogComponent";
import PurchaseOrdersCreateDialogComponent from "./PurchaseOrdersCreateDialogComponent";
import PurchaseOrdersFakerDialogComponent from "./PurchaseOrdersFakerDialogComponent";
import PurchaseOrdersSeederDialogComponent from "./PurchaseOrdersSeederDialogComponent";
...
const PurchaseOrdersPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("purchaseOrders");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , quotationID : urlParams.singleQuotationsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Purchase Orders", type: "error", message: error.message || "Failed get Purchase Orders" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("purchaseOrders").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Purchase Orders", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("purchaseOrders").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/purchaseOrders/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="purchaseOrders-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="purchaseOrders-datatable">
                    <PurchaseOrdersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <PurchaseOrdersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PurchaseOrdersCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <PurchaseOrdersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PurchaseOrdersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PurchaseOrdersPage);
