...
const PurchaseOrdersEditDialogComponent = (props) => {
...
POAmount: _entity?.POAmount,
        };

        setLoading(true);
        try {
            
        await client.service("purchaseOrders").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info purchaseOrders updated successfully" });
    ...
    return (
        <Dialog header="Edit Purchase Orders" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            style={{ maxWidth: "55vw" }} role="purchaseOrders-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(PurchaseOrdersEditDialogComponent);
