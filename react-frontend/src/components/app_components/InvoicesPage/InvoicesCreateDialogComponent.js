...
const [paymentTermsID, setPaymentTermsID] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [customerID,paymentTermsID], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            invoiceID: _entity?.invoiceID,customerID: _entity?.customerID?._id,invoiceDate: _entity?.invoiceDate,dueDate: _entity?.dueDate,totalAmount: _entity?.totalAmount,status: _entity?.status,paymentTermsID: _entity?.paymentTermsID?._id,remarks: _entity?.remarks,
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setPaymentTermsID(res.data.map((e) => { return { name: e['paymentTermID'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="invoiceDate"  value={_entity?.invoiceDate ? new Date(_entity?.invoiceDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("invoiceDate", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
