...
import InvoiceItemsDatatable from "./InvoiceItemsDataTable";
...
import InvoiceItemsFakerDialogComponent from "./InvoiceItemsFakerDialogComponent";
import InvoiceItemsSeederDialogComponent from "./InvoiceItemsSeederDialogComponent";
...
const InvoiceItemsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("invoiceItems");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , invoiceID : urlParams.singleInvoicesId ,invoiceItems : urlParams.singleServicesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Invoice Items", type: "error", message: error.message || "Failed get Invoice Items" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("invoiceItems").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Invoice Items", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("invoiceItems").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/invoiceItems/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="invoiceItems-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="invoiceItems-datatable">
                    <InvoiceItemsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <InvoiceItemsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <InvoiceItemsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <InvoiceItemsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <InvoiceItemsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(InvoiceItemsPage);
