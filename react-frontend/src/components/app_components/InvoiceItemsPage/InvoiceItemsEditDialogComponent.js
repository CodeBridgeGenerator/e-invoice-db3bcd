...
const InvoiceItemsEditDialogComponent = (props) => {
...
const [invoiceItems, setInvoiceItems] = useState([])
 ...
 useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setInvoiceItems(res.data.map((e) => { return { name: e['name'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
...
totalAmount: _entity?.totalAmount,
        ...
        await client.service("invoiceItems").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info invoiceItems updated successfully" });
...
const invoiceItemsOptions = invoiceItems.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Invoice Items" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="invoiceItems-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="unitPrice" className="w-full mb-3 p-inputtext-sm" value={_entity?.unitPrice} onChange={(e) => setValByKey("unitPrice", e.target.value)}  required  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(InvoiceItemsEditDialogComponent);
