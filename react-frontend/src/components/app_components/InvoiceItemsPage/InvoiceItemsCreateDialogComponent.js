...
const [invoiceItems, setInvoiceItems] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [invoiceID,invoiceItems], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            invoiceID: _entity?.invoiceID?._id,invoiceItems: _entity?.invoiceItems?._id,units: _entity?.units,unitPrice: _entity?.unitPrice,totalAmount: _entity?.totalAmount,
