...
const PaymentTermsEditDialogComponent = (props) => {
...
dueDays: _entity?.dueDays,
paymentTermID: _entity?.paymentTermID,
earlyPaymentDiscount: _entity?.earlyPaymentDiscount,
latePenalty: _entity?.latePenalty,
        };

        setLoading(true);
        try {
            
        const result = await client.service("paymentTerms").patch(_entity._id, _data);
        props.onHide();
...
export default connect(mapState, mapDispatch)(PaymentTermsEditDialogComponent);
