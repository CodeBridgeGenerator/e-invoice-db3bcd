...
const PaymentsCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.paymentMethod)) {
                error["paymentMethod"] = `Payment Method field is required`;
                ret = false;
            }
            ...
            if (_.isEmpty(_entity?.paymentStatus)) {
                error["paymentStatus"] = `Payment Status field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            paymentID: _entity?.paymentID,paymentMethod: _entity?.paymentMethod,dateIssued: _entity?.dateIssued,totalAmount: _entity?.totalAmount,invoiceID: _entity?.invoiceID?._id,paymentStatus: _entity?.paymentStatus,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="dateIssued"  value={_entity?.dateIssued ? new Date(_entity?.dateIssued) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("dateIssued", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="paymentStatus" className="w-full mb-3 p-inputtext-sm" value={_entity?.paymentStatus} onChange={(e) => setValByKey("paymentStatus", e.target.value)}  required  />
            </span>
            <small className="p-error">
