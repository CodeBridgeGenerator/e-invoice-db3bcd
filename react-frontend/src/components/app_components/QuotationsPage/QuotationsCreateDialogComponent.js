...
const QuotationsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            quotationID: _entity?.quotationID,customerID: _entity?.customerID?._id,quotationDate: _entity?.quotationDate,status: _entity?.status,totalAmount: _entity?.totalAmount,
