...
const ServicesCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.serviceID)) {
                error["serviceID"] = `Service ID field is required`;
                ret = false;
            }
            ...
            if (_.isEmpty(_entity?.serviceType)) {
                error["serviceType"] = `ServiceType field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            serviceID: _entity?.serviceID,name: _entity?.name,description: _entity?.description,units: _entity?.units,serviceType: _entity?.serviceType,price: _entity?.price,
    ...
    return (
        <Dialog header="Create Services" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <InputText id="serviceID" className="w-full mb-3 p-inputtext-sm" value={_entity?.serviceID} onChange={(e) => setValByKey("serviceID", e.target.value)}  required  />
            </span>
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="serviceType" className="w-full mb-3 p-inputtext-sm" value={_entity?.serviceType} onChange={(e) => setValByKey("serviceType", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["serviceType"]) ? (
              <p className="m-0" key="error-serviceType">
                {error["serviceType"]}
              </p>
            ) : null}
          </small>
